...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isMyImgPackage(file) ?
    `${outputDir}/MyImg.css` :
    `${outputDir}/style.css`;
...
function isMyImgPackage(file) {
  return path.normalize(file).includes('packages/MyImg/');
}


async function compileCSS() {
  ({ default: chalk } = await import('chalk'));
  const files = await glob('packages/{,@MyImg/}*/src/style.scss');
